...
void FileUtilsOhos::setassetmanager(NativeResourceManager* a) {
    if (nullptr == a) {
        OHOS_LOGD("setassetmanager : received unexpected nullptr parameter");
        return;
    }
...
FileUtils* FileUtils::getInstance() {
    OHOS_LOGD("[Nnnnut] FileUtils::getInstance");
    if (s_sharedFileUtils == nullptr)
    {
        ...
        if(!s_sharedFileUtils->init())
        {
          ...
          OHOS_LOGD("[Nnnnut] ERROR: Could not init FileUtilsOhos");
        }
    }
...
bool FileUtilsOhos::init() {
    OHOS_LOGD("[Nnnnut] FileUtilsOhos::init");
    _defaultResRootPath = "";
